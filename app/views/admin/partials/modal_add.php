...
<?php elseif($data['page'] == strtolower('condiciones')) : ?>
	...
	<div id="modal_add" class="hidden fixed inset-0 overflow-y-auto w-full h-screen bg-dark pt-24 px-2 py-5 bg-opacity-70">
		<form action="<?php echo URLROOT; ?>/admin/condiciones" method="post" autocomplete="off"  class="flex flex-col py-4 space-y-4 w-full mx-auto md:max-w-2xl bg-white shadow rounded-lg font-dmsans">
			...
			<div class="flex flex-col w-full px-4 space-y-2 ">
				...
				<div class="flex flex-col space-y-4 ">
					<div class="flex flex-col text-dark md:px-3 md:w-full">
						...
						<input type="text" name="numero" class="p-2 border outline-none rounded md:w-full" placeholder="Escribir numero" required>
					</div>

					<div class="flex flex-col text-dark md:px-3 md:w-full">
						<label for="titulo" class="md:w-full">Titulo </label>
						<input type="text" name="titulo" class="p-2 border outline-none rounded md:w-full" placeholder="Escribir titulo" required>
					</div>

					<div class="flex flex-col text-dark md:px-3 md:w-full">
						<label class="md:w-full">Descripcion </label>
		      	<textarea name="descripcion" rows="6"  class="p-2 border outline-none rounded md:w-full" placeholder="Escribe Descripcion " required></textarea>
		      </div>
			...
			<div class="flex flex-col w-full items-center text-lg font-bold md:flex-row md:justify-around md:space-y-0">
				<button name="create_condicion" type="submit" class="w-2/4 p-4 rounded-xl text-dark bg-ctaLight hover:bg-ctaDark md:w-1/2 border">Agregar Condicion</button>
			</div>
			<!-- end modal body -->
